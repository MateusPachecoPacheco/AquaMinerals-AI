...
Objetivos: Demonstrar que é possível conciliar desenvolvimento econômico com preservação ambiental através de tecnologia e inovação.`,
    keywords: ['projeto', 'objetivo', 'localização', 'madre de deus', 'bahia', 'monitoramento', 'pontos'],
...
Destino dos minerais: Indústria tecnológica, setor energético, materiais avançados, pesquisa científica, aplicações medicinais.`,
    keywords: ['minerais', 'sódio', 'magnésio', 'cálcio', 'potássio', 'lítio', 'extração', 'composição'],
    lastUpdated: '2024-01-15',
  },
  {
    id: 'sustentabilidade',
...
ODS 14 - Vida na Água: Contribuição direta para conservação dos oceanos e uso sustentável dos recursos marinhos.`,
    keywords: ['sustentabilidade', 'meio ambiente', 'impacto', 'ods', 'monitoramento', 'ph', 'temperatura', 'status'],
    lastUpdated: '2024-01-15',
  },
  {
    id: 'tecnologia',
...
Infraestrutura: Docker, CI/CD, Monitoramento e observabilidade.`,
    keywords: ['tecnologia', 'react', 'typescript', 'nodejs', 'ia', 'inteligência artificial', 'arquitetura', 'stack'],
    lastUpdated: '2024-01-15',
  },
  {
...
Benefícios para comunidades: Diversificação econômica, capacitação profissional, melhoria de infraestrutura, desenvolvimento regional.`,
    keywords: ['econômico', 'empregos', 'receita', 'royalties', 'investimento', 'desenvolvimento', 'comunidade'],
    lastUpdated: '2024-01-15',
  },
  {
...
function extractKeywords(text: string): string[] {
  ...
  const stopWords = new Set([
    ...
    'algo', 'nada', 'alguém', 'ninguém',
    'cada', 'outro', 'outros', 'outra', 'outras',
...
function classifyIntent(query: string): IntentClassification {
  ...
  const categoryKeywords: Record<KnowledgeCategory, string[]> = {
    projeto: ['projeto', 'objetivo', 'finalidade', 'missão', 'visão', 'onde', 'localização', 'bahia', 'madre'],
    sustentabilidade: ['sustentabilidade', 'ambiente', 'ambiental', 'eco', 'preservação', 'conservação', 'ods', 'verde'],
    minerais: ['mineral', 'minerais', 'extração', 'sódio', 'magnésio', 'cálcio', 'potássio', 'lítio', 'composição'],
    tecnologia: ['tecnologia', 'técnico', 'sistema', 'plataforma', 'software', 'ia', 'inteligência', 'artificial'],
    impacto_economico: ['econômico', 'economia', 'emprego', 'receita', 'lucro', 'investimento', 'custo', 'valor', 'dinheiro'],
    impacto_ambiental: ['impacto', 'dano', 'poluição', 'contaminação', 'emergência', 'risco', 'monitoramento', 'parâmetro'],
...
function searchKnowledge(query: SearchQuery): RetrievalResult[] {
  ...
  const results: RetrievalResult[] = KNOWLEDGE_BASE
    .filter(doc => {
...
function generateResponse(query: string, results: RetrievalResult[], intent: IntentClassification): RAGResponse {
  if (results.length === 0) {
    return {
      answer: "Desculpe, não encontrei informações específicas sobre isso na minha base de conhecimento atual. Posso ajudar com perguntas sobre o projeto AquaMinerals, minerais oceânicos, sustentabilidade, tecnologia utilizada, ou impactos econômico e ambiental.",
      sources: [],
      confidence: 0,
      followUpQuestions: [
  ...
  const relevantDocs = results.map(r => 
    KNOWLEDGE_BASE.find(d => d.id === r.documentId)
  ...
  if (intent.category === 'projeto') {
    followUpQuestions.push("Quais são os pontos de monitoramento?", "Como posso acessar o dashboard?");
  } else if (intent.category === 'minerais') {
    followUpQuestions.push("Qual é o destino dos minerais extraídos?", "A extração prejudica o meio ambiente?");
  } else if (intent.category === 'sustentabilidade') {
    followUpQuestions.push("Como é classificado o status ambiental?", "O projeto contribui para quais ODS?");
  }
